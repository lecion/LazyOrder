 ...
 * 广告轮播器
 */
public class AdvertiseRotator extends FrameLayout implements View.OnClickListener{
    ...
    public AdvertiseRotator(Context context) {
        super(context);
        this.context = context;
        this.adCount = 1;
        LayoutInflater.from(context).inflate(R.layout.view_ad_rotator, this);
    }

    public AdvertiseRotator(Context context, int adCount) {
        super(context);
        this.context = context;
     ...
     * @param attrs
     */
    public AdvertiseRotator(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        ...
        adCount = typeArr.getInt(R.styleable.adRotatorAttrs_count, 1);
    ...
    private void initDots() {
        ...
        if (llDotsContainer.getChildCount() != 0) llDotsContainer.removeAllViews();
        LayoutParams params;
        LinearLayout dot;
        for(int i = 0; i < adCount; i++) {
            dot = new LinearLayout(context);
    ...
    public void onClick(View view) {
        if(advertises != null && advertises.size() != 0) {
            ...
            if(advertise != null) {
                String advertiseType = advertise.getType();
                if (advertiseType.equals(ApiConstants.KEY_ADVERTISE_TYPE_NOTICE)) {
                ...
                } else if(advertiseType.equals(ApiConstants.KEY_ADVERTISE_TYPE_ADVERTISE)) {
